...
export class EmailService implements IEmailService {
    ...
    public async sendRateToSubcribers(): Promise<void> {
        ...
        try {
            const bitcoinRate: number =
                await this.bitcoinService.getBitcoinRate()
            emails.forEach((email: string) =>
                this.sendEmail(this.getRateEmailTemplate(email, bitcoinRate))
            )
        } catch (err) {
            console.log(
    ...
    private sendEmail(emailDetails: Mail.Options) {
        ...
        try {
            ...
            console.log(
                `An error occorred while trying to send the provided email: ${emailDetails}.`,
                err
            )
        }
    }

    private getRateEmailTemplate(
